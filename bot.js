...
async function generatePost(chatId, rawText, nicheId) {
  ...
  try {
    ...
    // data.post may be a raw JSON string from the upstream function
    // Extract the actual content field if so
    let post = data.post;
    if (typeof post === "string") {
      try {
        const parsed = JSON.parse(post);
        post = parsed.content || parsed.post || post;
      } catch (e) {
        // Not JSON, use as-is
      }
    }
    // Strip any metadata trailer
    if (typeof post === "string") {
      post = post.split(/-METADATA/)[0].trim();
    }
...
bot.on("message", async (msg) => {
  ...
  if (state.mode === "refine") {
    ...
    try {
      ...
      if (data.error) return bot.sendMessage(chatId, `Error: ${data.error}`);
      let refined = data.post || data.content;
      if (typeof refined === "string") {
        try {
          const parsed = JSON.parse(refined);
          refined = parsed.content || parsed.post || refined;
        } catch (e) {}
      }
      if (typeof refined === "string") {
        refined = refined.split(/-METADATA/)[0].trim();
      }
